...
char getCharacterFromHuffmanCode(TreeNode *root, const std::string &huffmanCode)
{
    ...
    for (char bit : huffmanCode)
    {
        ...
        if (currentNode->symbol != '\0') // We've reached a leaf node
        {
...
std::string decodeHuffmanString(TreeNode *root, const std::string &huffmanString)
{
    ...
    std::string currentHuffmanCode;
    for (char bit : huffmanString)
    {
        currentHuffmanCode += bit;
        char character = getCharacterFromHuffmanCode(root, currentHuffmanCode);
        if (character != '\0') // We've found a character
        {
            decodedString += character;
            currentHuffmanCode.clear();
        }
    }
...
int main()
{
    ...
    std::cout << "Concatenated Huffman string: " << huffmanString << std::endl;
        ...
        // Get Huffman string from user
    std::string huffmanInput;
    std::cout << "Enter a Huffman string: ";
    std::getline(std::cin, huffmanInput);

    // Decode Huffman string and print the result
    std::string decodedString = decodeHuffmanString(root, huffmanInput);
